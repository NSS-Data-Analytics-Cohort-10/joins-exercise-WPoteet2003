...
--Answer: Toy Story 4, 1,073,394,593
...
SELECT AVG(film_budget), company_name 
FROM specs
LEFT JOIN distributors
ON specs.domestic_distributor_id=distributors.distributor_id
LEFT JOIN revenue
...
LEFT JOIN rating
ON rating.movie_id = specs.movie_id
WHERE LOWER(headquarters) NOT LIKE '%, ca%';

--Answer: 2; Dirty Dancing at 7.0

-- 7. Which have a higher average rating, movies which are over two hours long or movies which are under two hours?

SELECT avg(imdb_rating),
CASE WHEN length_in_min<120 THEN 'SHORT'
ELSE 'LONG'
END type
FROM specs
LEFT JOIN rating
...
SELECT SUM(worldwide_gross), AVG(imdb_rating),
CASE 
